from ladybug.sunpath import LBSunpath, LBSun
...
class Sunpath(LBSunpath):
    ...
    Attributes:
        latitude: The latitude of the location. Values must be between -90 and
            90. Default is set to the equator.
        ...
        timeZone: A number representing the time zone of the location you are
            ...
            plot.  The time zone should follow the epw convention and should
            be between -12 and +12, where 0 is at Greenwich, UK, positive values
        ...
        daylightSavingPeriod: An analysis period for daylight saving.
            (Default: None)
        basePoint: A DS point that will be used as the center of sunpath.
            (Default: Origin)
    ...
    def __init__(self, latitude=0, northAngle=0, longitude=0, timeZone=0,
                 daylightSavingPeriod=None, basePoint=None, scale=1,
                 sunScale=1):
        """Init class."""
        # NOTE
        # The use of self.__class__ instead of name of subclass can be tricky in
        # case of creating a new subclass from sunpath unless it provides it's
        # own init but it was the only way that I could get it to work for now.
        # Read more here: http://stackoverflow.com/questions/576169/understanding-python-super-with-init-methods
        super(self.__class__, self).__init__(latitude, northAngle, longitude,
                                             timeZone, daylightSavingPeriod)

        basePoint = Point.Origin() if not basePoint else basePoint
    ...
    def fromLocation(cls, location, northAngle=0, daylightSavingPeriod=None,
        ...
        sp = super(Sunpath, cls).fromLocation(location, northAngle,
    ...
    @property
    # TODO: Update dictionary to namedtuple
    def geometries(self):
        """Return geometries."""
        return {
    ...
    def sunGeometries(self):
        """Get list of suns."""
        if not len(self.__suns):
            return self.__suns  # emapty list

        return tuple(sun.geometry for sun in self.__suns)
    ...
    def drawSunFromDateTime(self, dateTime, isSolarTime=False):
        ...
        self.drawSun(dateTime.month, dateTime.day, dateTime.floatHour,
                     isSolarTime)
    ...
    def calculateSunPositionFromDateTime(self, dateTime, isSolarTime=False):
        ...
        Returns:
            Returns a sun object
        """
        return self.calculateSunPosition(dateTime.month, dateTime.day,
                                         dateTime.floatHour, isSolarTime)

    def calculateSunPosition(self, month=12, day=21, hour=12, isSolarTime=False):
        """Calculate the position of sun based on origin and scale.

        Returns:
            A Ladybug sun object
        """
        ...
        sun = Sun.fromLBSun(lbSun, self.basePoint, self.__radius, self.scale,
                            self.sunScale)
    ...
    def calculateDailyCurve(self, month, day=21, isSolarTime=False):
        ...
        datetimesDictionary = self.calculateSunriseSunset(month, day=day,
                                                          depression=0,
        ...
        for datetime in datetimes:
            sun = self.calculateSunPositionFromDateTime(datetime,
                                                        isSolarTime=False)
    ...
    def __drawDirectionLines(self):
        ...
        for angle in range(0, 360, 90):
            self.__baseCurves.append(northArrow.Rotate(Plane.XY(),
                                                       angle + self.northAngle))
        ...
        for angle in range(0, 360, 30):
            if angle % 90 != 0:
                self.__baseCurves.append(
                    shortArrow.Rotate(Plane.XY(), angle + self.northAngle)
                )
...
class Sun(LBSun):
    ...
    def __init__(self, datetime, altitude, azimuth, isSolarTime,
                 isDaylightSaving, northAngle, sunpathBasePoint, sunpathScale,
                 sunpathRadius, sunScale):
        """Init class."""
        super(self.__class__, self).__init__(datetime, altitude, azimuth,
                                             isSolarTime, isDaylightSaving,
                                             northAngle)
    ...
    def fromLBSun(cls, LBSun, basePoint, scale, radius, sunScale):
        """Create from LBSun."""
        return cls(LBSun.datetime, LBSun.altitudeInRadians,
                   LBSun.azimuthInRadians, LBSun.isSolarTime,
                   LBSun.isDaylightSaving, LBSun.northAngle,
                   basePoint, scale, radius, sunScale)

    def __calculateGeometricalSun(self):
        """Calculate sun vector, base point and sphere."""
        # convert sun vector to Dynamo Vector
        self.__vector = Vector.ByCoordinates(self.sunVector.x,
                                             self.sunVector.y, self.sunVector.z)

        movingVector = self.__vector.Reverse().Scale(
            self.__sunpathRadius * self.__sunpathScale)
    ...
    def __createSunSphere(self):
        ...
        sp = Sphere.ByCenterPointRadius(self.__position,
                                        self.scale * self.__sunRadius)
        self.__geometry = sp
