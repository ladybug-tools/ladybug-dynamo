from ladybug.legendparameters import LBLegendParameters
import color
...
class LegendParameters(LBLegendParameters):
    ...
    Attributes:
        ...
        chartType: 0: continuous, 1: segmented, 2: ordinal.
            (Default: 0. Ordinal values can be strings and well as numericals)
        title:  Legend title. It's usually analysis unit
        font: An optional text string that sets the font of the text. Examples
            include "Arial", "Times New Roman" or "Courier" (all without quotations).
            The text input here can be any font that is on your computer but the
            font must be of an Editable file type (as seen in the font folder
            off of your control panel).  Font files that are Print and Preview
            will not work.  If you wish to use a Bold version of the font,
            include a ", Bold" at the end of the font name (example: "Arial,
            Bold").
        fontSize: An optional number to set the size of the text in model's units.
        scale: Input a number here to change the scale of the legend. (Default: 1)
        basePlane: Input a plane to change the location and orientation of the
            legend. The default is set to the right of the analysis scene in XY
            plane.
    ...
    def __init__(self, legendRange=('min', 'max'), numberOfSegments=11,
        ...
        """Init class."""
        LBLegendParameters.__init__(self, legendRange, numberOfSegments, colors,
                                    chartType)
    ...
    @property
    def colors(self):
        """Return list of colors in this legend parameters."""
